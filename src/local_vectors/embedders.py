...
class LocalEmbedder:
	...
	def set_device(self, device: str) -> None:
		'''
		Reset the device for the model to run on.
		@param: device (str), the new device the model will run on.
		@return: returns nothing.
		'''
		self.device = device
	...
	def set_batch_size(self, batch_size: int) -> None:
		'''
		Reset the batch size for the model to process inputs.
		@param: batch_size (int), the new batch size value.
		@return: returns nothing
		'''
		self.batch_size = batch_size
	...
	def embed_text(
	...
	) -> List[Dict[str, Union[np.array, int]]]:
		'''
		Embed the text string and return a list containing all 
			generated vectors and metadata associated with each vector
			as it relates to the original input string.
		@param text (str), the text string that is going to be 
			embedded.
		@param truncate (bool), whether to truncate the input string if
			it is too long for the model context length. Is "False" by
			default.
		@param to_binary (bool), whether to generate binary embeddings 
			along with the full floating point 32 ones. Is "False", by
			default.
		@param vectors_only (bool), whether to return only the vectors
			generated by the function (excluding all metadata). Is 
			"False" by default.
		@return: returns a List[Dict] where each dictionary contains 
			the vector generated by the function and the metadata for 
			each chunk of text that was embedded. The metadata 
			includes the index of the chunk with respect to the 
			original text and the length of the chunk. If vectors_only 
			is True, returns a List[Dict] where each dictionary only 
			contains the vector(s) generated by the function for each 
			chunk of text that was embedded.
		'''
		# Validate text is not an empty string.
		if text == "":
			raise ValueError("Expected non-empty string for text to be embedded")
		...
		# Concatenate batched embeddings back together.
		embeddings = np.concatenate(all_embeddings, axis=0)
		if to_binary:
			binary_embeddings = np.concatenate(all_binary_embeddings, axis=0)

		# Verify all embeddings were generated for each chunk in the 
		# metadata.
		...
		# If the user only wants the vectors without the paired 
		# metadata, update the chunk metadata without it.
		if vectors_only:
			chunk_metadata = [
				{
	...
	def batch_embed_text(
		...
		to_binary: bool = False,
		vectors_only: bool = False,	
	) -> List[List[Dict[str, Union[np.array, int]]]]:
		'''
		Embed a batch of text strings and return a list containing all 
			generated vectors and metadata associated with each vector
			as it relates to the original input string.
		@param text (List[str]), the text list that is going to be 
			embedded.
		@param truncate (bool), whether to truncate the input strings 
			if they are too long for the model context length. Is 
			"False" bydefault.
		@param to_binary (bool), whether to generate binary embeddings 
			along with the full floating point 32 ones. Is "False", by
			default.
		@param vectors_only (bool), whether to return only the vectors
			generated by the function (excluding all metadata). Is 
			"False" by default.
		@return: returns a List[List[Dict]] where each dictionary 
			contains the vector generated by the function and the 
			metadata for each chunk of text that was embedded. The 
			metadata includes the index of the chunk with respect to 
			the original text and the length of the chunk. If 
			vectors_only is True, returns a List[List[Dict]] where each 
			dictionary only contains the vector(s) generated by the 
			function for each chunk of text that was embedded.
		'''
		# Validate input is a list of strings.
		if not (isinstance(text, list) and all(isinstance(item, str) for item in text)):
			raise ValueError(f"Expected text List[str], received {type(text)}")

		all_chunk_metadata = []
		
		# Flatten all documents into chunks. Process the chunks 
		# sequentially to save on memory, then batch the inference.
		for idx, text_line in enumerate(text):
			if not text_line:
				continue

			# Generate chunks for this document.
			doc_chunks = vector_preprocessing(
				text_line, 
			...
			for chunk in doc_chunks:
				# Ensure all chunks are padded to the model's expected context length
				length_diff = self.model_metadata["max_tokens"] - len(chunk["tokens"])
				if length_diff != 0:
					chunk["tokens"].extend([self.tokenizer.pad_token_id] * length_diff)
				
				# Optional: track source document index to help the caller re-associate
				chunk["doc_batch_idx"] = idx
				all_chunk_metadata.append(chunk)

		if not all_chunk_metadata:
			return []

		# Extract tokens for batch inference.
		all_tokens = [chunk["tokens"] for chunk in all_chunk_metadata]
		all_embeddings = []
		all_binary_embeddings = []
		
		# Batch embed the global list of tokens.
		...
		# Concatenate batched results back together.
		embeddings = np.concatenate(all_embeddings, axis=0)
		if to_binary:
			binary_embeddings = np.concatenate(all_binary_embeddings, axis=0)

		# Map the vectors back to their respective metadata 
		# dictionaries.
		for idx, chunk in enumerate(all_chunk_metadata):
			# Remove tokens to free up memory before returning.
			if "tokens" in chunk:
				del chunk["tokens"]
		...
		# Apply final filter for vectors_only if requested.
		if vectors_only:
			all_chunk_metadata = [
				{k: v for k, v in chunk.items() if "vector" in k or k == "doc_batch_idx"}
